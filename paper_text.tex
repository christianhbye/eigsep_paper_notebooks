...
% N_ANCHOR and every number in blocks 1, 4, 5 and 6 -- it just no longer
...
%
% Blocks 1 and 2 are one continuous passage in "Minimising Covariance with the
% 21-cm Signal" and must be pasted in that order: block 1 establishes the
% filter and what the signal retains, block 2 asks what of that is the
% antenna, and the closing caveat paragraph at the end of block 2 shuts the
% section. Do not reinstate the caveat paragraph at the end of block 1 -- it
% used to live there, and with the comparison inserted after it, it read as
% closing the section twice.
%
% ===================================================================
...
% rather than the vocabulary -- the own-basis floors of block 5 -- the words
...
% these are three ANTENNAS with none. Blocks 1, 2, 5 and 6 quote numbers at
...
% block 5 quotes every position number at. They coincided until the 21-cm
...
%
% Block 5 quotes at NHAND because that is where the position error is the term
...
% That fragility is block 5's closing argument rather than an embarrassment --
...
%
% THE VIVALDI IS NOT A REJECTED CANDIDATE, and block 2 lives or dies on this.
...
% suspension flew in October 2024. So block 2 frames the comparison as a
...
% Replaces the passage from "Using a representative beam ..." up to but NOT
% including "... under realistic systematics and foreground filtering."; block
% 2 continues directly from it and carries the closing paragraph.
%
% Paragraph 1 is the manuscript's own and is untouched. Paragraph 2 differs
% from the original only in that its residuals are placeholders and that "To
% filter foregrounds to 10\,mK, we need to remove the first eight modes" is
% gone -- see the 10 mK note in the header. The residual at that depth survives
% as the dynamic range, which is the claim the sentence was making all along.
%
% "The central panel" throughout, not "the figure": Fig. 1 has three panels now
% and only the middle one is the EIGSEP antenna. Getting this wrong would
% attribute the bowtie's numbers to whichever panel the reader looks at first.
%
...
% collapses to an indistinguishable residual. One sentence answers that. Note
% it got weaker when the ensemble moved into antenna temperature -- PAIR1 was
% 99 per cent when the models were sky-referred -- and it is quoted honestly.
%
% Deliberately absent: the retention-vs-trough-depth correlations (detail; the
% notebook still prints them); the `stays below' rule; and the repeated "N is
% not a filter depth we propose to apply to data", which is block 5's argument
% and is made there, once. EDGES, MIST and REACH are not cited here either:
% the subsection introduces all three a few paragraphs earlier.
%
...
% and never touches zeus21.UVLF_binned. The README also asks for a link to the
% repository; it is a \footnote at first mention, which is what this manuscript
% does for CROISSANT and every other piece of software.
% ===================================================================
...
In the central panel of Fig.~\ref{fig:singular_values}, we present the residual
RMS after filtering as a function of the number of eigenmodes filtered, $N$.
...
The central panel shows that the filter explains foreground variance more
effectively than the cosmological signal, and that after $N=10$ modes are
removed, the RMS of the median retained 21-cm signal is larger (0.87\,mK)
than the RMS of the residual foregrounds (0.62\,mK).
...
% ===================================================================
% BLOCK 2 -- same section, immediately after block 1. Three paragraphs of
% comparison, then the closing paragraph that used to end block 1.
%
% Paragraph 3 is the destination: the design claim in
% section~\ref{subsec:eig_antenna} is what this block exists to substantiate.
% If the block has to be cut, cut towards that sentence, and keep the two
% framing clauses named in the header.
%
% The closing paragraph reads better here than it did at the end of block 1:
% "knowledge of the sky or the beam may allow some chromatic structure to be
% modelled rather than filtered" lands immediately after three panels
% demonstrating what beam chromaticity costs.
% ===================================================================

The amount of signal that survives this filter is a property of the antenna as
much as of the sky. The three panels of Fig.~\ref{fig:singular_values} repeat
the simulation and the filter for three beams observing the same sky behind the
same horizon: an isotropic beam, which has no chromaticity and so bounds what
any antenna could achieve at this site; the EIGSEP bowtie; and a Vivaldi feed
of the type used for the ground antennas (section~\ref{sec:vivaldis}) and flown
on the first suspension test. Each is filtered in its own eigenbasis and scaled
by its own open-sky fraction, since both are properties of the beam.
...
not required to deliver a low-chromaticity global-signal measurement.
...
% ===================================================================
% BLOCK 3 -- caption for beam_comparison.pdf, which is now Fig. 1. Keep
...
% reader who never reaches the text needs. The nuance is block 5's job. Do not
...
% ITS own basis comes back at OWNLO-OWNHI mK. That argument is block 5,
...
% phrase, and block 5 opens by refuting it. The vertical shift needs CLEAR_U
...
% ===================================================================
% BLOCK 5 -- section "Forward Modelling". Insert after "The tension in the
